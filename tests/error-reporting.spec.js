...
describe('# Build new layout', () => {
  const appDirname = path.join(process.cwd(), 'tests', 'test-error-reporting');
  ...
  it(`browser clients should launch properly`, function() {
    ...
    return new Promise(async resolve => {
      ...
      const element = await page.waitForSelector('pre code');
      const value = await element.evaluate(el => el.textContent);

      assert.isTrue(value.includes('Build failed with 1 error:'));
